...
#define LCD1602_4PINS //如果声明了 LCD1602_4PINS 代表使用4位并行接口方式
                      //否则使用8位并行接口方式
...
void LCDInit();//初始化
void LCDSetPosition(unsigned char x, unsigned char y);//确定显示位置
void LCDShowStr(unsigned char x, unsigned char y, unsigned char *str);//显示常规字符函数
void LCDWriteCGRAM(unsigned char *str);//显示用户自定义字符函数(1) 需修改
void LCDSetChar(unsigned char x, unsigned char y, unsigned char pos, unsigned char *str);//显示用户自定义字符函数(2)
